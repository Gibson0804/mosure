...
class InstallationService
{
    ...
    public function install(array $data)
    {
        try {
            ...
            // 6. 创建队列/任务相关基础表（database queue 场景）
            $this->createQueueInfrastructureTables();
    ...
    protected function createQueueInfrastructureTables(): void
    {
        try {
            StructuredLogger::info('install.queue_tables.start');

            if (! Schema::hasTable('jobs')) {
                StructuredLogger::info('install.table.create', ['table' => 'jobs']);
            ...
            if (! Schema::hasTable('failed_jobs')) {
                StructuredLogger::info('install.table.create', ['table' => 'failed_jobs']);
            ...
            if (! Schema::hasTable('job_batches')) {
                StructuredLogger::info('install.table.create', ['table' => 'job_batches']);
            ...
            StructuredLogger::info('install.queue_tables.completed');
        } catch (\Throwable $e) {
            StructuredLogger::error('install.queue_tables.failed', [
                'error' => $e->getMessage(),
            ]);
            throw $e;
        }
